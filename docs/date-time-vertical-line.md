# Date/Time Vertical Line Indicator

`date-time-vertical-line.pine` is a TradingView Pine Script indicator that draws one vertical line at a user-selected date and time.

## Features

- Select the target date and time from the indicator settings.
- Draw a vertical line that extends through the full chart pane.
- Customize line color, width, and style.
- Optionally show a label with custom text and the selected timestamp.

## Installation

1. Open TradingView and go to **Pine Editor**.
2. Copy the contents of `indicators/date-time-vertical-line.pine`.
3. Paste the code into Pine Editor.
4. Click **Save**.
5. Click **Add to chart**.

## Usage

1. Open the indicator settings.
2. Set **Date and time** to the timestamp you want to mark.
3. Adjust the visual options if needed.

TradingView interprets the selected timestamp using the chart/exchange timezone displayed in the platform.
